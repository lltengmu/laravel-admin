<template>
  <el-card class="box-card border-none" shadow="always">
        ...
        <section class="float-right">
          <el-dropdown trigger="hover">
            <button class="m-2 outline-none border px-2 py-1 rounded-md">
              <span class="text-lg font-bold">{{ caseStore.pagesCount }}</span>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-for="item in caseStore.pageCounts" @click="caseStore.setPageCount(item.value)">{{ item.value }}</el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        </section>
    ...
    <el-table ref="multipleTableRef" :row-style="caseStore.handleRowStyle"
      :header-row-style="caseStore.handleHeaderRowStyle" :highlight-current-row="true" :data="caseStore.caseTableData"
    ...
    <div class="flex justify-end mt-[10px]">
      <el-pagination background layout="prev, pager, next" :total="caseStore.casesList.length"
        @current-change="caseStore.handlePageChange" />
    </div>
  </el-card>
</template>
